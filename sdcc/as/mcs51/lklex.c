...
/*)Function	_IO_ssize_t	getasmline()
 *
 *	The function getasmline() reads a line of input text from a
 ...
 *	getasmline() returns a (1) after succesfully reading a line
...
ssize_t
getasmline(char **__restrict __lineptr, size_t *__restrict __n, FILE *__restrict __stream)
{
	register int ftype;
	register char *fid;

loop:
	if (pflag && cfp && cfp->f_type == F_STD)
