 ...
 *	or read from a command file.  The functiond getasmline() and parse()
 ...
 *		_IO_ssize_t	getasmline()	lklex.c
 *		VOID	library()	lklibr.c
 *		VOID	link_cm()		lkmain.c
...
char *argv[];
{
	...
	while (1) {
		ip = ib;					
		if (getasmline(NULL, NULL, NULL) == 0)
			break;
		if (pflag && sfp != stdin)
			fprintf(stdout, "%s\n", ip);
		if (*ip == '\0' || parse())
	...
	for (pass=0; pass<2; ++pass) {
		...
		while (getasmline(NULL, NULL, NULL)) {
			ip = ib;
			link_cm();
		}
		if (pass == 0) {
...
/*)Function	link_cm()
 *
 *	The function link_cm() evaluates the directives for each line of
...
VOID sym()
{
	...
	fprintf( mfp,	"; no$gmb format .sym file\n"
			"; Generated automagically by ASxxxx linker2 %s (SDK " SDK_VERSION_STRING ")\n"
		, VERSION );
	/*
